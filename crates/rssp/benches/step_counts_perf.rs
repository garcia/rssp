...
fn bench_step_counts_pipeline(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("step_counts_pipeline");
...
fn bench_step_counts_inner(c: &mut Criterion) {
    let (charts, globals) = build_count_inputs();
    let mut group = c.benchmark_group("step_counts_inner");
    group.sample_size(200);
    group.measurement_time(Duration::from_secs(2));
    group.bench_function("compute_step_counts", |b| {
        b.iter(|| {
            ...
            for chart in &charts {
                ...
                let mut timing_stats = rssp::stats::compute_timing_aware_stats(
                    black_box(&minimized_chart),
                    chart.lanes,
                    &timing,
                );
                timing_stats.total_steps = stats.total_steps;
                timing_stats.holding = stats.holding;
